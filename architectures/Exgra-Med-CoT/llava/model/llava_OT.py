...
class LlavaLlamaModel(LlamaModel):
    ...
    def initialize_vision_modules(self, model_args, vision_tower, mm_vision_select_layer,
        ...
        if self.contrastive:
            ...
            self.simi_type = getattr(model_args, 'simi_type', "average")
    ...
    def forward(
        ...
        ##################
        Fsentences_id: torch.LongTensor = None,
        ...
        if inputs_embeds is None:
            ...
            ##################
            if self.contrastive:
                Fsentences_embeds = self.embed_tokens(Fsentences_id)
            ##################
        ...
        if vision_tower is not None and (input_ids.shape[1] != 1 or self.training) and images is not None:
            ...
            if self.contrastive:
                print("-----------------after Decoder-------------")
                align_input_embeds = super(LlavaLlamaModel, self).forward(
                    input_ids=None, attention_mask=attention_mask_F, past_key_values=past_key_values,
                    inputs_embeds=Fsentences_embeds, use_cache=use_cache,
                ...
                if self.simi_type == 'average':
                    ...
                    for i, image_feat in enumerate(align_image_features):
                        ...
                        for j, align_input in enumerate(align_input_embeds):
                            u = torch.ones(image_feat.shape[0], device=image_features.device, dtype=image_features.dtype)/image_feat.shape[0]
                            v = torch.ones(align_input.shape[0], device=image_features.device, dtype=image_features.dtype)/align_input.shape[0]
                            ...
                            with torch.no_grad():
                                ...
                                T = self.Sinkhorn(KK,u,v)
                            if torch.isnan(T).any():
                                return None
                            
                            sim_op = torch.sum(T * simi)
                            matrixSimi[i, j] = (sim_op)*torch.exp(self.temperature)
                elif self.simi_type == 'un_ot':
                    align_image_features_copy = align_image_features.permute(1, 0, 2)
                    align_input_embeds_copy = align_input_embeds.permute(1, 0, 2)
                    
                    align_image_features_norm = F.normalize(align_image_features_copy, dim=2)
                    align_input_embeds_norm = F.normalize(align_input_embeds_copy, dim=2)

                    M = align_image_features_copy.shape[0]
                    N = align_input_embeds_copy.shape[0]
                    B = align_input_embeds_copy.shape[1]
                    sim = torch.einsum('mbd,ncd->mnbc', align_image_features_norm, align_input_embeds_norm).contiguous()  
                    print("sim before:", sim.shape)
                    sim = sim.view(M, N, B*B)
                    print("sim after:", sim.shape)
                    sim = sim.permute(2,0,1)
                    wdist = 1.0 - sim
                    with torch.no_grad():
                        ot_prob = LinearProblem(
                            wdist, epsilon=0.1, tau_a=self.tau, tau_b=self.tau
                        )
                        sinkhorn = Sinkhorn(lse_mode=True, use_danskin=True)
                        sol = sinkhorn(ot_prob)[0]
                        # print(sol)
                        T = sol.matrix
                    ...
                    print("sim_op shape:", sim_op.shape)
                    sim_op = sim_op.contiguous().view(B, B)
                    print("max_sim:", sim_op.max())
                    print("tau value:", self.tau)
                    matrixSimi = (sim_op)*torch.exp(self.temperature)

                elif self.simi_type == 'directOT':
                    for image_feat, align_input in zip(align_image_features, align_input_embeds):
                        image_feat_norm = image_feat/image_feat.norm(dim=1)[:, None]
                        align_input_norm = align_input/align_input.norm(dim=1)[:, None]
                        u = torch.ones(image_feat.shape[0], device=image_features.device, dtype=image_features.dtype)/image_feat.shape[0]
                        v = torch.ones(align_input.shape[0], device=image_features.device, dtype=image_features.dtype)/align_input.shape[0]
                        ...
                        if torch.isnan(T).any():
                            return None
                        lossAlign += torch.sum(T * cost)
                else:
                    raise ValueError("Wrong similarity type.")
                
                if self.contrastive_loss_type == "infonce" and self.simi_type != 'directOT':
                    ...
                    lossAlign = lossAlign
                elif self.contrastive_loss_type == "siglip" and self.simi_type != 'directOT':
                    ...
                    lossAlign = lossAlign
                elif self.contrastive_loss_type == "none" and self.simi_type == 'directOT':
        ...
        return super(LlavaLlamaModel, self).forward(
            ...
            output_attentions=output_attentions, output_hidden_states=output_hidden_states,
            return_dict=return_dict
        ), lossAlign, self.alpha, torch.exp(self.temperature), self.beta
...
class LlavaLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        outputs, lossAlign, alphaArg, tempPa, betaArg = self.model(
            input_ids=input_ids,
            #########################
            Fsentences_id=Fsentences_id,
            attention_mask_F=attention_mask_F,
            #########################
